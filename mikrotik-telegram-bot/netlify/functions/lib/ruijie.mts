...
async function creds(uid:number){const raw=await DB().get(`ruijie/${uid}`);return raw?unseal<{appid:string;secret:string}>(raw):null}
...
export async function handleRjCommand(cmd:string,args:string[],m:any):Promise<boolean>{
  ...
  if(cmd==="/logout"){await DB().delete(`ruijie/${uid}`);await say(chat,"Ruijie credentials cleared.");return true}
  try{
